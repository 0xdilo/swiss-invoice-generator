...
async def create_invoice(
    ...
    with db() as conn:
        c = conn.cursor()
        c.execute("INSERT INTO invoices (client_id, template_id, data) VALUES (?, ?, ?)",
                  (client_id, template_id, data))
        conn.commit()
...
def update_invoice(invoice_id: int, data: dict):
    with db() as conn:
        c = conn.cursor()
        c.execute("UPDATE invoices SET data=? WHERE id=?", (json.dumps(data), invoice_id))
        conn.commit()
        return {"ok": True}
